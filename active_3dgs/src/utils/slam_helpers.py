...
def get_depth_and_silhouette(pts_3D, w2c, clip_depth=False, depth_cutoff=10.0):
    """
    ...
    if clip_depth:
        # remove points with depth>depth_cutoff
        valid_depth = torch.where(depth_z < depth_cutoff)
        depth_z = depth_z[valid_depth]
        depth_z_sq = depth_z_sq[valid_depth]
        print("size of valid_depth: ", depth_z.shape)
        if depth_z.shape[0] == 0:
            return None

    # Depth and Silhouette
    depth_silhouette = torch.zeros((depth_z.shape[0], 3)).cuda().float()
...
def transformed_params2depthplussilhouette(params, w2c, transformed_gaussians, clip_depth=False, depth_cutoff=10.0):
    # Check if Gaussians are Isotropic
    if params['log_scales'].shape[1] == 1:
        log_scales = torch.tile(params['log_scales'], (1, 3))
    else:
    ...
    rendervar = {
        'means3D': transformed_gaussians['means3D'],
        'colors_precomp': get_depth_and_silhouette(transformed_gaussians['means3D'], w2c, clip_depth, depth_cutoff),
        'rotations': F.normalize(transformed_gaussians['unnorm_rotations']),
        'opacities': torch.sigmoid(params['logit_opacities']),
        'scales': torch.exp(log_scales),
        'means2D': torch.zeros_like(params['means3D'], requires_grad=True, device="cuda") + 0
...
def transformed_params2rendervar_multi(params, transformed_gaussians):
    # Check if Gaussians are Isotropic
    if params['log_scales'].shape[1] == 1:
        log_scales = torch.tile(params['log_scales'], (1, 3))
    else:
...
def transformed_params2rendervar_multi_split(params, transformed_gaussians):
    # Check if Gaussians are Isotropic
    if params['log_scales'].shape[1] == 1:
        log_scales = torch.tile(params['log_scales'], (1, 3))
    else:
    ...
    rendervar1 = {
        'means3D': transformed_gaussians['means3D'],
        'colors_precomp': params['features'][:, :12],
        'rotations': F.normalize(transformed_gaussians['unnorm_rotations']),
        'opacities': torch.sigmoid(params['logit_opacities']),
        'scales': torch.exp(log_scales),
        'means2D': torch.zeros_like(params['means3D'], requires_grad=True, device="cuda") + 0
...
def transform_to_frame_batch(params, cam_unnorm_rots, cam_trans, gaussians_grad, camera_grad):
    """
    Function to transform Isotropic or Anisotropic Gaussians from world frame to camera frame.
    ...
    Returns:
        transformed_gaussians: Transformed Gaussians (dict containing means3D & unnorm_rotations TODO)
    """

    if camera_grad:
        cam_rot = F.normalize(cam_unnorm_rots)
        cam_tran = cam_trans
    else:
        cam_rot = F.normalize(cam_unnorm_rots.detach())
        cam_tran = cam_trans.detach()
    rel_w2c = torch.eye(4).unsqueeze(0).repeat(cam_rot.shape[0], 1, 1).cuda().float()
    rel_w2c[:, :3, :3] = build_rotation(cam_rot)
    rel_w2c[:, :3, 3] = cam_tran
    ...
    pts_ones = torch.ones(pts.shape[0], 1).cuda().float()
    pts4 = torch.cat((pts, pts_ones), dim=1)
    transformed_pts = (rel_w2c @ pts4.T).permute(2, 1, 0)[:, :3]
    transformed_gaussians = transformed_pts.permute(2, 0, 1)
    # # Transform Rots of Gaussians to Camera Frame
    # if transform_rots:
    #     norm_rots = F.normalize(unnorm_rots)
    #     transformed_rots = quat_mult(cam_rot, norm_rots)
...
def ai2thor_transform_to_frame(params, gaussians_grad, camera_grad):
    """
    Function to transform Isotropic or Anisotropic Gaussians from world frame to camera frame.
